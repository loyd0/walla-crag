    ...
    return (
        <nav className={`flex justify-between max-w-5xl mx-auto items-center py-12 ${className}`}>
            ...
            <ul className="md:flex justify-end m-0 hidden">
                {links.map(link => <li key={link.text} className="mr-4 m-0 hover:text-blue-700"> <a href={link.linkTo}>{link.text}</a>  </li>) } 
            </ul>
            <i className={`md:hidden mr-10 text-3xl fa text-black z-10 ${showMenu ? "fa-times text-blue-700" : "fa-bars text-white"}`} onClick={() => setShowMenu(!showMenu)}></i>
